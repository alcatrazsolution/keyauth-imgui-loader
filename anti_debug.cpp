...
//		if (FindWindow((LPCWSTR)name, 0)) { return security::internal::debug_results::find_window; }
//	}
//
//	return security::internal::debug_results::none;
//}
...
//	CreateMutex(NULL, FALSE, (LPCWSTR)get_string(5));
...
//	if (GlobalGetAtomName(INVALID_ATOM, (LPTSTR)buffer, 1) != FALSE || GetEnvironmentVariable((LPCWSTR)get_string(6), (LPWSTR)buffer, 4096 * 4096) != FALSE || GetBinaryType((LPCWSTR)get_string(7), (LPDWORD)buffer) != FALSE
...
//	HKEY h_key = 0;
//	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, skCrypt(L"HARDWARE\\ACPI\\DSDT\\VBOX__"), 0, KEY_READ, &h_key) == ERROR_SUCCESS) { return security::internal::debug_results::check_registry; }
//
//	return security::internal::debug_results::none;
//}
//
//int security::internal::virtualization::vm() {
//	if (CreateFile(skCrypt (L"\\\\.\\VBoxMiniRdrDN"), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, 0, OPEN_EXISTING, 0, 0) != INVALID_HANDLE_VALUE) { return security::internal::debug_results::vm; }
//
//	if (LoadLibrary(skCrypt (L"VBoxHook.dll"))) { return security::internal::debug_results::vm; }
//
//	HKEY h_key = 0;
//	if ((ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, skCrypt (L"SOFTWARE\\Oracle\\VirtualBox Guest Additions"), 0, KEY_READ, &h_key)) && h_key) { RegCloseKey(h_key); return security::internal::debug_results::vm; }
//
//	h_key = 0;
//	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, skCrypt (L"HARDWARE\\DESCRIPTION\\System"), 0, KEY_READ, &h_key) == ERROR_SUCCESS)
//	{
//		unsigned long type = 0;
//		unsigned long size = 0x100;
//		char* systembiosversion = (char*)LocalAlloc(LMEM_ZEROINIT, size + 10);
//		if (ERROR_SUCCESS == RegQueryValueEx(h_key, skCrypt (L"SystemBiosVersion"), 0, &type, (unsigned char*)systembiosversion, &size))
//		{
...
//		if (ERROR_SUCCESS == RegQueryValueEx(h_key, skCrypt(L"VideoBiosVersion"), 0, &type, (unsigned char*)videobiosversion, &size))
//		{
...
//					if (strstr(video, skCrypt ("oracle")) || strstr(video, skCrypt ("virtualbox"))) { return security::internal::debug_results::vm; }
...
//	}
//
//	HANDLE h = CreateFile(skCrypt (L"\\\\.\\pipe\\VBoxTrayIPC"), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0);
...
//	if ((ERROR_SUCCESS == RegOpenKeyEx(HKEY_LOCAL_MACHINE, (LPCWSTR)w_subkey, 0, KEY_READ, &h_key)) && h_key)
//	{
...
//					{
//						if ((RegOpenKeyEx(h_key, (LPCWSTR)w_key_new, 0, KEY_READ, &h_new_key) == ERROR_SUCCESS) && h_new_key)
//						{
//							unsigned long nn = 0;
...
//							{
//								HKEY HKKK = 0;
//								if (RegOpenKeyEx(h_new_key, (LPCWSTR)w_key_newer, 0, KEY_READ, &HKKK) == ERROR_SUCCESS)
//								{
//									unsigned long size = 0xFFF;
//									unsigned char value_name[0x1000] = { 0 };
//									if (RegQueryValueEx(h_new_key, skCrypt (L"FriendlyName"), 0, 0, value_name, &size) == ERROR_SUCCESS) { to_lower(value_name); if (strstr((char*)value_name, skCrypt ("vbox"))) { return security::internal::debug_results::vm; } }
//									RegCloseKey(HKKK);
//								}
//							}
